...
contract RarityCraftingMaterialsIMarket_1 is Initializable {
  using EnumerableSet for EnumerableSet.AddressSet;
  using SafeCast for uint;

  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
  event Modify(address indexed lister, uint indexed price, int amount, uint indexed summonerId);
  event Buy(
  ...
  function modify(
    ...
    int newAmount = oldAmount.toInt256() + _amount;
    require(newAmount >= 0, 'bad amount');
    uint oldValue = prices[msg.sender] * oldAmount;
    uint newValue = _price * uint(newAmount);

    prices[msg.sender] = _price;
    amounts[msg.sender] = uint(newAmount);
  ...
  function buy(
    ...
    require(_minPrice <= price, '!minPrice');
    uint buyValue = price * _buyAmount;
    require(msg.value >= buyValue);
    require(RM.ownerOf(_summonerId) == msg.sender, '!owner');
    ...
    emit Buy(_lister, msg.sender, price, _buyAmount, _summonerId, _minPrice);
  }
